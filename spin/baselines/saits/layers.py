...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        if attn_mask is not None:
            # this mask is imputation mask, which is not generated from
            # each batch, so needs broadcasting on batch dim
...
class EncoderLayer(nn.Module):
    def __init__(self, d_time, d_feature, d_model, d_inner, n_head, d_k, d_v,
    ...
    def forward(self, x: Tensor):
        if self.diagonal_attention_mask:
            mask_time = torch.eye(self.d_time).to(x.device)
        else:
            mask_time = None

        residual = x
        # here we apply LN before attention cal, namely Pre-LN
        enc_input = self.layer_norm(x)
        enc_output, attn_weights = self.slf_attn(enc_input, enc_input,
                                                 enc_input, attn_mask=mask_time)
